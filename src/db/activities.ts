...
const UPDATABLE_FIELDS = new Set([
  ...
  'synced_from', 'last_sync_at', 'external_id', 'is_shared',
]);
...
export async function createActivity(activity: Omit<Activity, 'id' | 'created_at'>): Promise<number> {
  ...
  const result = await db.execute(
    `INSERT INTO activities (title, activity_type, description, date, time_start, time_end,
     location, max_participants, actual_participants, animator_name, status, materials_needed, notes, linked_project_id, is_shared)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
